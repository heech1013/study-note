...
여러 프로세스를 처리해야 하는 과정에서, 현재 진행 중인 task의 상태를 PCB에 저장하고, 다음에 진행할 task의 상태값을 읽는 과정을 의미한다.

> - (조금 더 간결하게 정의하자면) 현재 진행하고 있는 프로세스를 멈추고, 다른 프로세스에게 CPU를 할당해 작업을 수행하는 과정이다.
...
> Context Switching에는 오버헤드가 발생한다. 그럼에도 번거롭게 수행하는 이유는, 오버헤드가 발생함에도 CPU 활용성을 높이는 것이 더 이득이기 때문이다. 이를 `Administrative Overhead` 라고도 표현한다.
...
여러 스레드에 대해, 하나의 자원에 대한 사용 권한을 주거나 이용 순서를 조정하는 기법.
...
> 뮤텍스는 화장실(공유 자원), 화장실 키(Counter/lock), 화장실을 이용하려는 사람(스레드)을 비유로 이해할 수 있다.
...
각 프로세스로 하여금 그들만의 전용 메모리가 있다고 착각하게 만드는 메모리 관리 기법.
