...
- 어플리케이션의 상태가 뷰를 업데이트. (JS -> HTML)
- 뷰는 상태가 업데이트 되도록 이벤트를 발생시키며, 직접 수정하지는 않는다.

> - 데이터 바인딩: 두 데이터의 소스를 일치시키는 기법.
>   - 웹 개발에서의 데이터 바인딩은 화면에 보이는 데이터와 브라우저 메모리의 데이터(즉, 여러개의 JavaScript 객체)를 일치시키는 것을 말한다.
>   - 또 다른 예로, MVC 모델에서 view(보여지는 부분)와 model(데이터)의 소스를 일치시키는 기법.
...
> - 상위 화면 요소: HTML 트리 구조 상으로 한 단계 위에 있는 요소
> - 브라우저는 특정 화면 요소에서 이벤트가 발생했을 때, 그 이벤트를 최상위에 있는 화면 요소(`body` 태그)까지 전파시킨다.

---

### 이벤트 캡쳐란?

특정 화면 요소에서 이벤트가 발생했을 때, 최상위 요소인 `body` 태그에서 이벤트가 발생한 태그를 찾아 내려가는 특성.

---

### 이벤트 캡쳐를 구현하는 방법은?

addEventListener()의 옵션 파라미터 객체에 `capture: true`를 설정해주면 된다.
...
- 하위 요소 각각에 이벤트를 붙이는 경우, 새로운 하위 요소가 추가되었을 때 일일이 이벤트를 새로 등록해주어야 한다. (이벤트 위임을 사용할 경우 그러지 않아도 된다.)
...
가장 많이 사용되는 모듈 번들러 중 하나이다.

---

### 모듈 번들러란?

웹을 구성하는 자원을 각각 모듈로 보고, 해당 모듈들을 조합해서 하나의 결과물을 만드는 도구.

> - 웹을 구성하는 자원에는 HTML/CSS, JavaScript, Images 등이 있다.
> - 하나의 시작점(entry point, 엔트리)로부터, 의존적인 모듈을 전부 찾아내 하나의 결과물을 만들어낸다.

---

### 모듈이란?

- (프로그래밍 관점에서는) 특정 기능을 갖는 작은 코드 단위.
- (웹팩의 관점에서는) (코드 단위의 모듈에 국한되지 않고) 웹을 구성하는 모든 자원.

---

### 모듈 번들러(혹은 웹팩)이 필요한 이유는?

1. 파일 단위로 자바스크립트 모듈을 관리해야 할 필요성이 생겼기 때문이다.
2. 웹을 개발할 때 반복되는 작업들을 자동화할 수 있기 때문이다.
3. 웹 로딩 속도를 개선할 수 있기 때문이다.

---

### 파일 단위로 모듈을 관리하지 않을 때의 문제점?

자바스크립트의 변수는 기본적으로 전역 스코프를 가진다.

따라서 웹을 개발할 때, 이를테면 HTML 파일에서 두 개 이상의 자바스크립트 모듈을 로딩하여 사용할 때, 변수를 중복 선언하거나 의도치 않은 값을 할당할 수 있다는 문제가 있다.

> 파일 단위로 모듈을 관리하기 이전에는, 자바스크립트 파일을 순서에 맞게 `<script>`로 로드하고, 미리 약속된 네임 스페이스에 담아 공유했다. 이런 방식은 프로젝트가 커지고 참여하는 개발자가 많아질수록 문제를 일으켰다.

---

### 웹팩이 파일 단위로 모듈을 관리하기 위해 사용하는 방법은?

AMD나 Common JS 방식의 모듈 관리 환경을 사용한다.

> - 웹팩은 AMD 방식의 모듈 관리 환경인 RequireJS를 오랫동안 사용해오다, CommonJS 방식의 Browserify로 변경했다.
> - CommonJS 방식으로 바꾸게 된 이유는, AMD 방식의 경우 모듈을 로드할 때 모듈을 사용하고자 하는 함수의 인자로 넘겨야 하다 보니 모듈이 많아질수록 관리하기 힘들기 때문이다.
> - 또한, Node.js가 점점 더 많이 사용되면서, Node.js가 사용하는 모듈 관리 방식인 CommonJS 한가지로 통합하고 싶었기 때문이다.
> - 웹팩은 AMD와 CommonJS 방식 둘 다 지원한다.
...
- 여러 모듈을 하나의 번들 파일로 묶음으로서, 브라우저가 다운 받는 자원의 수를 줄인다.
- Lazy Loading을 지원해서, 필요한 자원을 그때그때 로드하도록 한다.
...
> - 의존성 그래프: 의존성에 따라 이어진 모듈 간 연결들의 집합
...
```html
<!-- 번들된 결과물을 다음과 같이 index.html 파일에서 script 태그로 로드한다. -->
...
자바스크립트가 아닌 파일들을 웹팩이 이해할 수 있게끔 변경해주는 역할을 하는 것.

> - 예를 들어, 로더는 타입스크립트와 같은 다른 언어들을 자바스크립트 문법으로 변경해주고, 이미지를 data URL 형식의 문자열로 변환해주고, CSS 파일을 자바스크립트에서 직접 로딩할 수 있도록 도와준다.

---

### 로더의 예시에는 어떤 것들이 있을까?

- `babel-loader`: ES6+에서 작성된 자바스크립트 파일을 ES5로 변환해준다.
- `css-loader`: CSS 파일을 자바스크립트 파일로 변환해서 로드해준다.
- `style-loader`: 자바스크립트로 변환된 스타일시트를 동적으로 DOM에 추가해준다. 보통 CSS를 번들하기 위해 `css-loader`와 함께 사용된다.

---

### 플러그인(plugin)이란?

(로더는 파일 단위 모듈을 처리하는 반면-) 플러그인은 번들된 결과물을 처리해주는 것.

> - 플러그인의 예시로, 자바스크립트 결과물을 난독화해주는 `UglifyJsPlugin` 등이 있다.
> - 플러그인을 사용하기 위해서는 웹팩 설정 객체의 `plugins` 배열에 추가한다.

---

### 바벨이란?

ES6(혹은 ES2015) 이상의 스펙으로 작성된 코드를, 모든 브라우저에서 동작하도록 (적당한 하위 버전으로) 변환해주는 트랜스파일러.
...
대표적으로 AMD와 CommonJS가 있다.

> - 질문을 보면 알겠지만, AMD와 CommonJS는 자바스크립트 모듈을 구현하는 것을 목적으로 하는 '명세'(spec)이다.
> - 자바스크립트는 다른 언어들과 달리 모듈 사용의 표준이 존재하지 않는다.
...
- 자바스크립트를 사용하는 모든 환경(서버사이드, 데스크탑 어플리케이션 등)에서 모듈을 사용하는 것을 목표로 한다.
- `exports` 키워드로 모듈을 만들고, `require()` 함수로 불러들이는 방식을 사용한다.

> - CommonJS는 실질적인 표준(de facto standard) 역할을 하고 있다.
> - CommonJS를 따르는 대표적인 프로젝트로는 Node.js가 있다.

AMD(Asynchronous Module Definition)

- 비동기로 로딩되는 환경(주로 브라우저 환경)에서 모듈을 사용하는 것을 목표로 한다.

> - AMD를 구현한 모듈 로더로는 대표적으로 RequireJS가 있다.
>
> - UMD(Universal Module Definition)은 AMD를 기반으로 CommonJS 방식까지 지원하는 통합 형태이다.
...
- IIFE(즉시실행함수) 방식의 모듈을 사용
- AMD, CommonJS 방식의 모듈 관리 환경을 사용

> ES Module이 등장하기 전까지 실제로 위와 같은 방법을 사용했다.

---

### IIFE 방식으로 모듈을 사용하는 방법은?

1. 네임 스페이스를 만들어서, 같은 이름의 변수라도 다른 네임 스페이스에 존재하는 변수일 경우 겹치지 않도록 한다.
2. IIFE로 네임 스페이스에 변수를 생성한다.

> - 네임 스페이스: 개체를 구분할 수 있는 범위를 나타내는 말. 하나의 네임 스페이스 공간 내에서 하나의 이름은 단 하나의 개체를 가리키게 된다.

(example)

```jsx
const math = math || {}; // 네임 스페이스 math
...
> - 즉시실행함수로 감쌌기 때문에, 다른 파일에서 해당 함수의 스코프 안으로 접근할 수 없다.
...
화면을 이루는 Element 그룹을 반환하는 JavaScript 함수이다.
...
- 하나의 기능(ex: 팝업을 띄운다)을 하는 코드가 컴포넌트 내 여러군데에 퍼져있지 않도록 커스텀 훅 등으로 뭉친다.

> - 하나의 기능을 하는 코드를 뭉친다고 무조건 좋은 것은 아니다. 코드 파악에 필수적인 핵심 정보까지 뭉치면 안되고(: 커스텀 훅의 대표적인 안티 패턴), 당장은 몰라도 되는 디테일만 뭉쳐야 한다.
> - 핵심 정보(ex: 팝업 제목, 내용, 액션)는 바깥에서 넘기고, 구현 디테일만 커스텀 훅 안에 숨긴다
> - 이는 '선언적 프로그래밍'의 방식과 일치한다. '무엇'을 하는 함수인지 알려주고, '어떻게' 구현하는지는 숨긴다. 코드만 보고도 빠르게 파악할 수 있으며, '무엇'만 바꿔서 쉽게 재사용할 수 있다는 장점이 있다.

2. 단일 책임: 하나의 일을 하는 뚜렷한 이름의 함수를 만든다.

- 함수: 한가지 일만 하는, 명확한 이름의 함수를 만든다.
- 컴포넌트: 한가지 일만 하는, 기능성 컴포넌트를 활용한다.

  ```jsx
  // example
  // before: 버튼 클릭 함수에 로그 찍는 함수와 API 콜이 섞여있다.
  <button
    onClick={async () = >{
      log('제출 버튼 클릭')
      await openConfirm()
...
> - 무조건적인 원칙이 아니고, 레퍼런스를 보며 개인적으로 정리한 원칙임을 밝힘.
> - reference: 토스ㅣSLASH 21 - 실무에서 바로 쓰는 Frontend Clean Code (https://www.youtube.com/watch?v=edWbHp_k_9Y)
